...
def taylor(x):
    ...
    if x == 0:
      for i in range(50000001):
        ...
        for j in range(50000001, 70000001):
          soma2 += 1/j
    elif x == 2:
        for j in range(70000001, 100000001):
          soma3 += 1/j
...
if __name__ == '__main__':
    executor = concurrent.futures.ProcessPoolExecutor(3) #A declaração with é usada para criar um executor de instância do ThreadPoolExecutor que irá esvaziar os threads imediatamente após a conclusão.
    futures = []
    for x in range(3):
    ...
    print("Série de Taylor paralelizada com {} Threads = {}".format(3,sum))